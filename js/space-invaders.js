...
WelcomeState.prototype.draw = function(game, dt, ctx) {
    ...
    ctx.fillText("太空侵略者", game.width / 2, game.height/2 - 40); 
    ctx.font="16px Arial";

    ctx.fillText("按下鍵盤上的「空白鍵」或觸碰開始遊戲。", game.width / 2, game.height/2); 
};
...
GameOverState.prototype.draw = function(game, dt, ctx) {
    ...
    ctx.textAlign="center"; 
    ctx.fillText("遊戲結束！", game.width / 2, game.height/2 - 40); 
    ctx.font="16px Arial";
    ctx.fillText("您得到 " + game.score + " 分且來到關卡 " + game.level, game.width / 2, game.height/2);
    ctx.font="16px Arial";
    ctx.fillText("按下鍵盤上的「空白鍵」重新遊戲。", game.width / 2, game.height/2 + 40);   
};
...
PlayState.prototype.draw = function(game, dt, ctx) {
    ...
    ctx.font="14px Arial";
    ctx.fillStyle = '#ffffff';
    var info = "生命值：" + game.lives;
    ctx.textAlign = "left";
    ctx.fillText(info, game.gameBounds.left, textYpos);
    info = "分數：" + game.score + "，關卡：" + game.level;
    ctx.textAlign = "right";
...
LevelIntroState.prototype.draw = function(game, dt, ctx) {
    ...
    ctx.textBaseline="middle"; 
    ctx.textAlign="center"; 
    ctx.fillText("關卡 " + this.level, game.width / 2, game.height/2);
    ctx.font="24px Arial";
    ctx.fillText("倒數 " + this.countdownMessage + " 秒", game.width / 2, game.height/2 + 36);      
    return;
};
